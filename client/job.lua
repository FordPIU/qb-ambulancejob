...
RegisterNetEvent('qb-ambulancejob:elevator_roof', function()
    ...
    for i,v in pairs(Config.Locations["roof"])do
        DoScreenFadeOut(500)
        while not IsScreenFadedOut() do
            Wait(10)
        end

        local dco = vector3(v[1], v[2], v[3])
        if #(pco - dco) < 1.0 then
            h = i
            break
        end
    ...
    local coords = Config.Locations["main"][h]
...
RegisterNetEvent('qb-ambulancejob:elevator_main', function()
    ...
    for i,v in pairs(Config.Locations["main"])do
        DoScreenFadeOut(500)
        while not IsScreenFadedOut() do
            Wait(10)
        end
    ...
    local coords = Config.Locations["roof"][h]
    SetEntityCoords(ped, coords.x, coords.y, coords.z, 0, 0, 0, false)
    SetEntityHeading(ped, coords.w)
